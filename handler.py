...
class Scheduler(object):
    ...
    def __init__(self, config: Dict) -> None:
        ...
        # IoT sensor, module
        if self.is_use_flag("module", "relay_module", "scheduler") or self.is_use_flag(
            "module", "relay_module", "conditions"
        ):
        ...
        if self.is_use_flag("sensor", "bh1750fvi"):
            if "light" not in self._params:
                self._params["light"] = {
                    ...
                    "max": 0,
                    "count": 0,
                    "function": self._bh1750fvi_sensor.get_light,
                }
        if self.is_use_flag("sensor", "sht31"):
            if "temperature" not in self._params:
                self._params["temperature"] = {
                    ...
                    "max": 0,
                    "count": 0,
                    "function": self._sht31_sensor.get_temperature,
                }
            if "humidity" not in self._params:
                self._params["humidity"] = {
                    "value": 0,
                    "average": 0,
                    "min": 0,
                    "max": 0,
                    "count": 0,
                    "function": self._sht31_sensor.get_humidity,
                }
        if self.is_use_flag("sensor", "co2mini"):
            if "temperature" not in self._params:
                self._params["temperature"] = {
                    ...
                    "max": 0,
                    "count": 0,
                    "function": self._co2mini_sensor.get_temperature,
                }
            if "humidity" not in self._params:
                self._params["humidity"] = {
                    "value": 0,
                    ...
                    "max": 0,
                    "count": 0,
                    "function": self._co2mini_sensor.get_humidity,
                }
            if "co2" not in self._params:
                self._params["co2"] = {
                    ...
                    "max": 0,
                    "count": 0,
                    "function": self._co2mini_sensor.get_co2,
                }
        if self.is_use_flag("sensor", "vl6180"):
            if "light" not in self._params:
                self._params["light"] = {
                    ...
                    "max": 0,
                    "count": 0,
                    "function": self._vl6180x_sensor.get_light,
                }
            if "distance" not in self._params:
                self._params["distance"] = {
                    ...
                    "max": 0,
                    "count": 0,
                    "function": self._vl6180x_sensor.get_distance,
                }
    ...
    def _init_spread_sheet(self) -> None:
        ...
        if not self.is_use_flag("google", "spread_sheet"):
            return
        used_params = [
            self.params_mapping[key]["name"] for key in self._get_params_key_list()
        ]
        if not used_params:
            return
    ...
    def _reduce_interval_minutes(self) -> None:
        ...
        self._interval_minutes["relay_module"] = max(
            0, self._interval_minutes["relay_module"] - interval_minutes
        )
        for key in self._params.keys():
            self._interval_minutes["alert_mail"][key] = max(
                0, self._interval_minutes["alert_mail"][key] - interval_minutes
            )
    ...
    def _fetch_params(self) -> None:
        ...
        for key, data in self._params.items():
            ...
            if count == 0 or max < sensor:
                self._params[key]["max"] = sensor
            if count == 0 or sensor < min:
                self._params[key]["min"] = sensor
    ...
    def _logging_spread_sheet(self) -> None:
        ...
        append_rows = [current_datetime] + [
            round(self._params[key]["value"], 1) for key in self._params.keys()
        ]
        if self.is_use_flag("module", "relay_module", "conditions"):
    ...
    def _is_power_flag(self) -> bool:
        ...
        for conditions_filter in self._config["module"]["relay_module"]["conditions"][
            "filters"
        ]:
            ...
            if (
                conditions_filter["limit"] == "upper"
                and sensor_value >= filter_threshold
            ):
                return True
            if (
                conditions_filter["limit"] == "lower"
                and sensor_value <= filter_threshold
            ):
                return True
    ...
    def _turn_on_power(self) -> None:
        ...
        if not self.is_use_flag(
            "module", "relay_module", "scheduler"
        ) and not self.is_use_flag("module", "relay_module", "conditions"):
            return

        self._relay_module.setup()
        self._relay_module.turn_on(
            self._config["module"]["relay_module"]["turn_on_minutes"]
        )
        self._interval_minutes["relay_module"] = self._config["module"]["relay_module"][
            "conditions"
        ]["skip_interval_minutes"]
    ...
    def _alert_mail(self) -> None:
        ...
        body = ""
        alert_mail_interval = self._config["google"]["mail"]["alert"][
            "skip_interval_minutes"
        ]
        for alert_filter in self._config["google"]["mail"]["alert"]["filters"]:
            filter_name = alert_filter["name"]
            ...
            if alert_filter["limit"] == "upper" and sensor_value >= filter_threshold:
                body += (
                    self._config["google"]["mail"]["alert"]["body"]["upper"].format(
            ...
            if alert_filter["limit"] == "lower" and sensor_value <= filter_threshold:
                body += (
                    self._config["google"]["mail"]["alert"]["body"]["lower"].format(
                        name=filter_name.capitalize(),
                        threshold=filter_threshold,
                        value=sensor_value,
                        unit=self.params_mapping[filter_name]["unit"],
    ...
    def _append_google_link_body(
        self, with_spread_sheet: bool = False, with_photo_library: bool = False
    ) -> str:
    ...
    def _send_mail(
        self, subject: str, body: str, image_file_list: Optional[List[str]] = None
    ) -> None:
        """Send the email.
        ...
        """
        if not self.is_use_flag("google", "mail", "summary") and not self.is_use_flag(
            "google", "mail", "alert"
        ):
            return
        ...
        if image_file_list:
            ...
            message = self._mail_client.create_message_with_image(
                self._config["google"]["mail"]["to_address"],
                subject,
                body,
                image_file_list,
            )
    ...
    def summary_mail_job(self) -> None:
        ...
        if not self.is_use_flag("google", "mail", "summary") and (
            not self.is_use_flag("google", "spread_sheet")
            and not self.is_use_flag("google", "photo_library")
        ):
            return

        graph_image_path = (
            f"{self._config['google']['photo_library']['img_dir']}/graph.jpg"
        )
        ...
        body = (
            self._config["google"]["mail"]["summary"]["body"]["title"].format(
                from_days=from_days
            )
            + "\n"
        )
        ...
        body += self._append_google_link_body(
            with_spread_sheet=True, with_photo_library=True
        )
        self._send_mail(
    ...
    def _save_summary_graph(self, graph_image_path: str) -> bool:
        ...
        fig, axes = plt.subplots(
            ncols=num_cols, nrows=num_rows, sharex="col", figsize=(15, 10)
        )
        if params_len == 5:
    ...
    def _get_history_dataframe(self) -> Optional[pd.DataFrame]:
        ...
        history_df = history_df.astype(
            {
                self.params_mapping[key]["name"]: self.params_mapping[key]["type"]
                for key in self._get_params_key_list()
            }
        )

        target_date = (
            datetime.now()
            - timedelta(days=self._config["google"]["mail"]["summary"]["from_days"])
    ...
    def _save_photo_image(self) -> List:
        ...
        for filename in target_name_list:
            ...
            # imgフォルダに保存
            photo_image_path = (
                f"{self._config['google']['photo_library']['img_dir']}/{filename}"
            )
    ...
    def logging_avg_job(self) -> None:
        ...
        append_rows = [target_date_format] + [
            round(self._params[key]["average"], 1) for key in self._params.keys()
        ]
...
def main() -> None:
    ...
    if scheduler.is_use_flag("google", "mail", "summary"):
        _create_scheduler_job(
            scheduler.summary_mail_job, config["google"]["mail"]["summary"]["scheduler"]
        )
    if scheduler.is_use_flag("module", "relay_module", "scheduler"):
        _create_scheduler_job(
            scheduler.relay_module_job, config["module"]["relay_module"]["scheduler"]
        )
    if scheduler.is_use_flag("module", "web_camera_module"):
        _create_scheduler_job(
            scheduler.web_camera_module_job,
            config["module"]["web_camera_module"]["scheduler"],
        )
    ...
    if "interval_minutes" in scheduler_config:
        ...
        for day_of_week in scheduler_config["day_of_week"]:
            if "at_time" in scheduler_config:
                at_time_list = (
                    [scheduler_config["at_time"]]
                    if isinstance(scheduler_config["at_time"], str)
                    else scheduler_config["at_time"]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="main",
        help="set function name in this file",
    )
